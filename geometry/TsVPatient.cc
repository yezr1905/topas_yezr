...
TsVPatient::TsVPatient(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM,
					   TsVGeometryComponent* parentComponent, G4VPhysicalVolume* parentVolume, G4String& name)
	: TsBox(pM, mM, gM, parentComponent, parentVolume, name),
...
G4VPhysicalVolume* TsVPatient::Construct() {
	...
	if (fIsCopy) {
		...
		if (!fImagingToMaterial) {
			...
			if (converterNameLower == "materialtagnumber") {
