...
//fAddUnitEvenIfItIsOne(false), fUID(0)
TsScoringManager::TsScoringManager(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM, TsFilterManager* fM)
	:fPm(pM), fMm(mM), fGm(gM), fFm(fM),
	fAddUnitEvenIfItIsOne(false), fUID(0)
{
#ifdef TOPAS_MT
...
{
	TsVScorer* scorer = fScoringHub->InstantiateScorer(fPm, fMm, fGm, this, scorerName, quantityName, outFileName, isSubScorer);
