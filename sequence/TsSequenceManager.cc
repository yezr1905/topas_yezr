...
TsSequenceManager::TsSequenceManager(TsParameterManager* pM, TsMaterialManager* mM, TsGeometryManager* gM, TsPhysicsManager* phM, TsVarianceManager* vM,
	TsFilterManager* fM, TsScoringManager* scM, TsSourceManager* soM,
	G4int argc, char** argv)
	: fPm(pM), fMm(mM), fGm(gM), fPhm(phM), fVm(vM), fFm(fM), fScm(scM), fSom(soM), fUseQt(false), fRunID(-1),
	fKilledTrackEnergy(0.), fKilledTrackCount(0), fUnscoredHitEnergy(0.), fUnscoredHitCount(0),
	fParameterizationErrorEnergy(0.), fParameterizationErrorCount(0), fIndexErrorEnergy(0.), fIndexErrorCount(0), fInterruptedHistoryCount(0),
	fIsExecutingSequence(false)
{
...
//#endif
	...
	TsActionInitialization* actionInitialization = new TsActionInitialization(fPm, fGm, fSom, fFm,fVm, this);
#ifdef TOPAS_MT
	...
	/*if (fUseQt) {
		...
		if (UIQt) fTsQt = new TsQt(fPm, fMm, fGm, fScm, this, fGrm, fSom);
...
#ifdef _WIN32
		Sleep(sleepInterval / ms); // In Windows, unit is ms
#else
		sleep(sleepInterval / s); // In Unix, unit is s
#endif
